...
const dataSelector = () => (req, res, next) => {
	try {
		...
		req.body = {service: service.id, end, begin, master: master.id, customer: customer.id}
		next()
	} catch (e) {
...
router.put('/:id', validator(orders), dateCreator(), dataSelector(), dbTryCatch(update))
router.post('/', validator(orders), dateCreator(), dataSelector(), dbTryCatch(add))
